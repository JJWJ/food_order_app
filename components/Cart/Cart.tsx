...
const Cart = (props: AppProps) => {
  ...
  const submitOrderHandler = async (userData: Inputs) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://react-food-app-88148-default-rtdb.firebaseio.com/orders.json",
        {
          method: "POST",
          body: JSON.stringify({
            user: userData,
  ...
  const modalActions = (
    <div className="text-right">
      <button
        className="cursor-pointer bg-transparent border border-solid border-red-700 py-2 px-4 rounded-3xl ml-4 text-red-700 hover:bg-red-800 hover:border-red-800 hover:text-white"
      ...
      {hasItems && (
        <button
          className="cursor-pointer bg-transparent border border-solid border-red-700 py-2 px-4 rounded-3xl ml-4 text-white bg-red-700 hover:bg-red-800 hover:border-red-800 hover:text-white"
  ...
  const isSubmittingModalContent = (
    <React.Fragment>
      <section className="text-center text-black text-xl flex justify-center items-center">
        <span>
          <svg
            className="animate-spin mr-4 h-5 w-5"
  ...
  const submittingErrorModalContent = (
    <React.Fragment>
